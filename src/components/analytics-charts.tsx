...
import { deviceStats, geoStats, scanSeries } from "@/lib/demo-data";
import { Panel } from "@/components/ui";

export function AnalyticsCharts({ hasData = true }: { hasData?: boolean }) {
  if (!hasData) {
  ...
  return <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
    ...
    <Panel><div className="mb-4"><p className="text-sm font-medium text-zinc-500">Device mix</p><h2 className="text-2xl font-semibold tracking-tight">Scan context</h2></div><ResponsiveContainer width="100%" height={190}><PieChart><Pie data={deviceStats} dataKey="value" innerRadius={52} outerRadius={78} paddingAngle={3}>{deviceStats.map((_, i) => <Cell key={i} fill={["#10b981", "#18181b", "#a1a1aa"][i]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer><div className="grid gap-2 text-sm">{deviceStats.map((item) => <div key={item.name} className="flex justify-between"><span>{item.name}</span><span className="font-semibold">{item.value}%</span></div>)}</div></Panel>
