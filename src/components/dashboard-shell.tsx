  ...
  return <div className="min-h-[100dvh] bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
    ...
    <div className="mx-auto grid max-w-7xl gap-4 p-3 md:grid-cols-[240px_1fr] md:p-5">
      <aside className="rounded-[2rem] border border-zinc-200 bg-white p-4 dark:border-white/10 dark:bg-zinc-900 md:sticky md:top-5 md:h-[calc(100dvh-2.5rem)]">
        <Link href="/" className="mb-6 flex items-center gap-2 font-semibold"><span className="grid size-9 place-items-center rounded-xl bg-zinc-950 text-white dark:bg-white dark:text-zinc-950"><MagicWand size={18} weight="bold" /></span>QRSpark</Link>
        ...
        <div className="mt-6 rounded-3xl bg-zinc-100 p-4 text-sm dark:bg-zinc-950"><p className="font-semibold">{user?.emailAddresses?.[0]?.emailAddress || "Workspace"}</p><p className="mt-1 text-zinc-500">Free plan: 1 dynamic QR included.</p></div>
      </aside>
