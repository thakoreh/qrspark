...
export function SmartRedirect() {
  ...
  return <Panel className="grid gap-4">
    ...
    <div className="flex flex-wrap gap-2"><button className="rounded-full border border-zinc-200 px-4 py-2 text-sm font-semibold dark:border-white/10" onClick={() => setRows([...rows, { url: "https://brand.example/new", weight: 10 }])}>Add variant</button><Button onClick={() => toast.info("Demo rules previewed. Connect Supabase to persist redirect rules.")}>Preview redirect rules</Button></div>
  </Panel>;
}
