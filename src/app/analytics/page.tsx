...
const stats = [
  [ChartLineUp, "1,247", "scans this month"],
  [DeviceMobile, "842", "unique devices"],
  [CursorClick, "97", "tracked conversions"],
  [MapPin, "12", "active placements"],
] as const;
  ...
  return <div className="min-h-[100dvh] bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50"><Nav />
    <main className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
      <section className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        ...
        <Panel className="p-5"><div className="grid gap-3 sm:grid-cols-2">{stats.map(([Icon, value, label]) => <div key={label} className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-white/10 dark:bg-zinc-950"><Icon size={24} className="text-emerald-600"/><p className="mt-5 text-3xl font-semibold">{value}</p><p className="text-sm text-zinc-500">{label}</p></div>)}</div></Panel>
      </section>
